...
설치기는 내부적으로 64비트 PowerShell을 우선 사용하고,
MSIX 설치 직후 COM 활성화가 지연될 때를 대비해 자동 재시도(1회)를 수행합니다.

## 필수 런타임 (중요)

Windows 11 1차 우클릭 COM 핸들러는 `.NET 6` 런타임이 필요합니다.
...
- `integration/installer/배프레임 레지스트리 등록.cmd`
  - (파일명이 깨지는 환경이면) `integration/installer/BAEFRAME-Registry-Install.cmd`를 대신 사용해도 됩니다.

이 파일은 내부적으로 `setup-paths.team.json`에 적힌 `shareAppPath`를 사용해 설치합니다.

중요: `installer` 폴더만 따로 압축해서 전달하면 설치가 실패합니다.
...
참고: JSON에서 Windows 경로에 `\\`를 쓰는 게 헷갈리면 `/`로 적어도 됩니다.
예: `C:/BAEframe/BAEFRAME/dist/win-unpacked/BFRAME_alpha_v2.exe`
