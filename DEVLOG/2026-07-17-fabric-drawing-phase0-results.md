...
| Fabric.js                 | `7.4.0`                                                                                                                                                                |
| 시험 도구                 | Node.js built-in test runner, 실제 `MPVOverlayHost` + fake `BrowserWindow`, 실제 Fabric pilot controller/runtime + 기존 테스트와 같은 fake Fabric Canvas/Path/DOM 방식 |
...
| 자동 검증 항목           | 결과 | 증거 범위                                                                                                                                                                                     |
| ------------------------ | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 임시 `.bframe` no-save   | PASS | 조작 전후 SHA-256과 파일 크기 동일. mtime도 추가 canary로 동일                                                                                                                                |
| 저장 호출 장부           | PASS | `saveReview` 0, `ReviewDataManager.save` 상당 호출 0                                                                                                                                          |
| 재생 명령 장부           | PASS | `mpvLoad`, `mpvPlay`, `mpvPause`, `mpvDestroyEmbed`, `mpvDestroyOverlay` delta 0                                                                                                              |
...
| playback/timeline canary | PASS | playback owner, media instance, duration, current frame, playlist continuous offset 동일                                                                                                      |
...
| 진단 validator           | PASS | playback/save/host/file/owner/media/timeline/controller/stale 필드를 각각 변조한 table test가 모두 위반으로 거부                                                                              |
