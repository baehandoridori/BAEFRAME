# 드로잉 전역 실행취소·UI 정식화 구현 개발 로그 (2026-08-28)

> 유일 사양: `C:\BAEframe\BAEFRAME_수정계획_드로잉-실행취소-UI정식화.md` (약 3,700줄, 독립 검증 2패스).
> 계획서 작성 기록: `DEVLOG/2026-08-28-드로잉-실행취소-UI정식화-계획서-작성기록.md`.
> 기준 스냅샷: `main` HEAD `0e674f6` (v2.4.3-beta). 테스트 기준선 **2,166 pass / 0 fail**.

## 요약

| 작업 | 문서 | 수정 파일 | 이유 | PR | 상태 |
|------|------|----------|------|-----|------|
| 1 | §7.1 | mpv-overlay-host.js, mpv-fabric-overlay-runtime.js, iife 번들, 테스트 2 | 오버레이 창이 키보드 포커스를 가진 적이 없어 Alt 드래그가 `altKey:false`로 도착 | [#188](https://github.com/baehandoridori/BAEFRAME/pull/188) | ✅ 완료 |
| — | — | package.json, package-lock.json, runtime-profile.test.js | 실기 확인용 패치 버전 상향 | [#189](https://github.com/baehandoridori/BAEFRAME/pull/189) | ✅ 완료 |
| 1-b | §7.1 후속 | mpv-fabric-overlay-runtime.js, iife 번들, 테스트 1 | 실기 보고 — HUD가 굵기를 보여주지 않아 알아채지 못함 | [#191](https://github.com/baehandoridori/BAEFRAME/pull/191) | ✅ 완료 |
| 2 | §6 | mpv-fabric-overlay-runtime.js, drawing-command-history.js, mpv-overlay-host.js, iife 번들, 테스트 12 | 히스토리가 씬별이라 재생헤드가 키프레임에 있어야만 반응 | [#193](https://github.com/baehandoridori/BAEFRAME/pull/193) | ✅ 완료 |
| 2-b | §6 후속 | mpv-fabric-overlay-runtime.js, iife 번들, 테스트 3+5 | 실기 보고 — 키프레임 생성이 실행취소 대상이 아니라 앞 획을 되돌려도 복사본이 남음 | [#196](https://github.com/baehandoridori/BAEFRAME/pull/196) | ✅ 완료 |
| 3 | §7.2 | (예정) | 도구 6종 신설 | ③ | ⬜ 대기 |
| 4 | §7.3 | (예정) | 지우개 도구·모드 섹션 | ③ | ⬜ 대기 |
| 5 | §7.4 | (예정) | `[` / `]` 브러시 크기 단축키 | ③ | ⬜ 대기 |
| 6 | §7 | (예정) | 팔레트 재구성 + 버전 | ③ | ⬜ 대기 |

---

## 작업 1 — Alt 브러시 크기 조절 복구 (PR ①)

### 배경

v2.4.3-beta 실기에서 Alt를 누른 채 드래그해도 브러시 크기가 변하지 않았다.
단위 테스트는 통과하고, **Ctrl 드래그 임시 지우개는 정상 동작**했다. 이 비대칭이 결정적 단서였다.

### 근본 원인

Windows 마우스 메시지(`WM_LBUTTONDOWN` 등)의 `wParam`에는 `MK_CONTROL`·`MK_SHIFT`가 실려 오지만
**Alt에 해당하는 플래그가 없다.** Chromium은 Alt 상태를 마우스 메시지가 아니라 **그 창(스레드)의
키보드 입력 큐**에서 얻는다. 오버레이가 키보드 포커스를 가진 적이 없으면 Alt keydown이 그 큐에
들어간 적도 없으므로, Alt+드래그가 `altKey:false`로 도착해 `isAltActive()`가 `false`를 돌려주고
크기 조절 분기에 진입하지 못한다.

`main/mpv-overlay-host.js`의 두 경로가 비대칭이었다.

- `updateDrawingTool()`은 도구 변경이 확정되면 **이미 `hostWindow.focus()`를 호출**하고 있었다.
- 그런데 그리기 입력을 켜는 `_setNativeDrawingInput(hostWindow, true)`에는 `focus()`가 없었다 —
  `setFocusable(true)`로 "포커스 가능"하게만 만들고 실제로 주지 않았다.

즉 **그리기 중 오버레이가 포커스를 갖는 것은 이미 의도된 정상 상태였는데, 진입 경로만 그것을 보장하지 않았다.**

### 배제한 원인 (다시 조사하지 말 것)

문서 `docs/drawing-keyframe-features.md` §7.1이 적은 착수점 4가지 중 3가지를 배제했다.

| 착수점 | 판정 | 근거 |
|---|---|---|
| ② `before-input-event`가 Alt를 삼키는가 | **아니다** | `FORWARDED_NAMED_KEY_CODES`에 `AltLeft`/`AltRight`가 없어 `createForwardedKeyboardInput()`이 `null`을 반환하고 `preventDefault`도 일어나지 않는다 |
| (추가) 번들이 소스보다 낡았는가 | **아니다** | 재생성 후 diff 0, 제스처 코드 22건 존재 |
| ④ HUD가 오버레이 밖에 그려지는가 | **아니고 증상과도 무관** | HUD `zIndex:3` > 팔레트 `zIndex:2`. 증상은 "HUD가 안 보인다"가 아니라 "크기가 안 변한다"였다 |

### 수정

**(a)** `_setNativeDrawingInput` enable 경로 마지막에 `hostWindow.focus?.()` 추가.
`moveTop()`·`invalidate()` **뒤**에 둔다 — 앞에 두면 네이티브 확장 스타일 변경이 포커스를 도로 가져갈 수 있다.

**(b)** 제스처 진단 계측. 문서 §7.1은 *"`getDiagnostics().gestures`로 관측 가능"*이라고 적었으나
**사실이 아니었다.** `getDrawingDiagnostics()`가 런타임 진단을 필드 단위 화이트리스트로 중계하는데
`gestures` 블록이 목록에 없어 **통째로 버려지고 있었다.** 실기에서는 애초에 아무것도 볼 수 없었다.

- 호스트에 `sanitizeFabricGestureDiagnostics()` 신설 — 값 범위를 고정한 채 중계
- 런타임에 `gestureProbe` 추가 — 오버레이 Alt/Ctrl keydown 횟수 + 마지막 pointerdown의 modifier 출처
  (`altKey` / 래치 / `pointerType` / `documentHasFocus` / 재생 여부)

**표본 채취 순서가 계약이다.** `syncOverlayModifierStateFromPointer()`가 마우스 경로에서 래치를 지우므로
`recordPointerdownProbe()`를 **그보다 먼저** 불러야 "래치가 있었는지"가 남는다. 테스트가 이 순서를 고정한다.

### 다음 재현에서 원인을 가르는 표

| `gestures.lastPointerdown.altKey` | `gestures.overlayAltKeyDownCount` | 결론 |
|---|---|---|
| `true` + `altSizeAdjustActive: true` | 무관 | 이번 수정으로 해결됨 |
| `false` | `0` | 오버레이가 Alt keydown을 받은 적 없음 → 포커스 문제 |
| `false` | 1 이상 | 오버레이는 Alt를 받는데 마우스 이벤트에만 안 실림 → Windows 메시지 계층 |

### 계획서와 실제가 달랐던 지점

없다. 계획서의 before 블록은 전건 문자 단위로 일치했고, **계획서가 예고한 기존 테스트 1건만 정확히 실패했다.**

`scripts/tests/mpv-overlay-host.test.js`의 `overlay.focus` 호출 횟수를 **정확히 1**로 세는 단언이다.
enable 경로가 포커스를 되돌리게 되면 2가 된다. **단언을 완화하지 않고** 기대값을 2로 올리고 설명 문자열을
갱신했다 — 이 단언은 "포커스 복원이 몇 번 일어나는가"를 세는 계약이고, 숫자가 늘어나는 것이 수정의 의도다.

신규 테스트를 작성하며 계획서 대비 2가지를 조정했다.

1. 진단 중계 테스트를 **"정상 중계"와 "`gestures` 부재 시 `null`" 두 건으로 분리**했다. 경계를 더 정확히 고정한다.
   그래서 신규가 계획서의 5건이 아니라 **6건**이 됐고, 부록 B 델타를 실측치로 정정했다.
2. `enabling drawing input focuses the overlay` 테스트에서 `moveTop`은 창 수명 초반(ensure/show)에도
   불리므로 `names.indexOf('moveTop', ignoreFalseIndex)`로 **enable 구간 이후만** 보게 했다.

### 결과

- `test:fabric-drawing-pilot` 465 → **468 pass / 0 fail**
- `test:mpv` 334 → **337 pass / 0 fail**
- 25스위트 전체 **2,166 → 2,172 pass / 0 fail**
- `npm run lint` error 0 (warning 59는 전부 기존)
- `npm run bundle:mpv-fabric-overlay` 재생성본 포함, 재실행 후 diff 0

---

## 배포 기록 (v2.4.4-beta)

| 항목 | 값 |
|------|-----|
| 배포 시각 | **2026-08-28 18:27** |
| 버전 | v2.4.4-beta (exe `FileVersion` = `2.4.4-beta`) |
| 소스 커밋 | `1f17040` (main, PR #189 머지) |
| 빌드 명령 | `npm run build` (`BAEFRAME_RUNTIME_PROFILE=fabric-v3-stable electron-builder --dir`) |
| 배포 경로 | `G:\공유 드라이브\JBBJ 자료실\한솔이의 두근두근 실험실\BAEFRAME\테스트버전 빌드` |
| 배포 방식 | `robocopy /MIR` — **exit 1(정상)**, 74개 복사 / 8개 동일 / FAILED 0 |
| 검증 | 전체 트리 SHA-256 대조 **82/82, MismatchCount=0 / MissingCount=0 / ExtraFiles=0** |
| exe SHA-256 | `6971D798385D9F3BC8EB57E1A6AF4137CC4442B76625C48DCC4CCD5C96A4622E` (176,886,784 bytes) |
| app.asar SHA-256 | `6DC541D4C544090842310CEF7CD0E8693607B730E21F2634EA6ECB5F56C8BFBA` (34,395,898 bytes) |
| 직전 배포본 | v2.4.3-beta, 2026-08-27 23:31 (교체됨) |

**배포 전 공유 드라이브 exe로 앱이 실행 중이어서 대기했다.** 직전 라운드의 `robocopy exit 9`가 같은 원인이었다.
사용자가 종료한 뒤 진행했고, 그 덕분에 `FileVersion` 조회도 캐시 없이 `2.4.4-beta`로 정확히 읽힌다.

### 실기 확인 필요 (부록 A의 A-1 ~ A-5)

이번 배포의 핵심 목적은 **Alt 원인 확정**이다.

1. `B`로 그리기 모드 진입 → **곧바로** Alt 누른 채 좌우 드래그 → 크기가 변하고 `NNpx` HUD가 뜬다
2. Alt 우클릭 드래그 → 컨텍스트 메뉴가 뜨지 않고 크기만 바뀐다
3. Ctrl 드래그 → 기존대로 지나간 획이 지워진다 (회귀 없음)
4. Alt 드래그 직후 일반 드래그 → 획이 정상적으로 그려진다 (래치 stuck 없음)
5. 그리기 모드에서 `Space`·`←`/`→` → 메인 창 단축키가 여전히 동작한다 (포커스 획득의 부작용 확인)

1번이 되면 원인이 확정되고 수정도 끝난 것이다. 여전히 안 되면 위 판별표로 다음 조치를 정한다.

---

## 다음 단계

PR ② Ctrl+Z 전역 실행취소 → PR ③ 도구 6종·지우개 모드·`[`/`]`·팔레트 재구성.
라운드 종료 후 `docs/drawing-keyframe-features.md` 갱신(계획서 부록 C-6).

---

# 작업 1-b — Alt 크기조절 HUD 원형 복원 (PR ①b, v2.4.5-beta)

## 사용자 보고

v2.4.4-beta 실기 확인 결과: **"크기는 변하는데, 화면 중앙에 오버레이는 나오지 않네."**

→ **작업 1의 근본 원인(오버레이 창의 키보드 포커스 부재)이 확정됐다.** Alt 크기 조절이 실제로 동작한다.
남은 것은 표시 문제였다.

## 이건 렌더링 버그가 아니었다 — 실측으로 배제

추측하지 않고 실제 Electron 렌더러에 오버레이를 띄워 측정했다(일회성 프로브, 커밋하지 않음).

| 측정 | 결과 |
|---|---|
| DOM 존재 | ✅ |
| `display` / `visibility` / `opacity` | `block` / `visible` / `1` |
| 좌표·크기 | (316, 222) 43.7×20 — 뷰포트 안 |
| 조상 사슬 | `transform`/`filter`/`contain`/`will-change` 없음 → `position: fixed` 정상 해석 |
| 페인트 순서 | `hudIsTopmost: true` — fabric 캔버스 **위** |
| 캡처 픽셀 | HUD 중심 RGB(82,82,84) α240, 좌측 끝 RGB(22,22,25) α235 = `rgba(24,24,28,0.92)` |

게다가 HUD는 팔레트(`z-index 2`)보다 위(`z-index 3`)다. **팔레트가 보이는 이상 HUD도 그려진다.**

### 프로브가 두 번 오진할 뻔했다

1. `document.elementFromPoint`가 fabric 캔버스를 돌려줘 "HUD가 캔버스 밑"이라는 결론이 가능했다.
   실제로는 HUD가 `pointer-events: none`이라 **히트테스트에서 건너뛰어진 것**이었다.
   잠시 `pointer-events: auto`로 되돌려 재측정해 `hudIsTopmost: true`를 확인했다.
2. 픽셀 샘플링이 캡처 배율을 무시했다. 창은 800×600인데 캡처는 1200×900(디바이스 배율 1.5×) —
   CSS 좌표로 읽어 α4의 빈 픽셀이 나왔다. 배율을 곱한 뒤에야 α240의 칩 픽셀이 나왔다.

## 진짜 원인 — 생김새

레거시 `.brush-size-hud`(`renderer/styles/main.css:2314`)와 비교하면 명확하다.

| | 레거시 | 파일럿(수정 전) |
|---|---|---|
| 형태 | **실제 브러시 굵기만 한 원** — 브러시 색 50% 알파 채움 + 브러시 색 테두리 | 44×20 검은 텍스트 칩 |
| 라벨 | 원 위에 `NNpx` (`::after`) | 칩 안 텍스트 |
| 앵커 | 드래그 시작점, `translate(-50%,-50%)` 중심 정렬 | 시작점 + (16, −28) |

이 제스처의 목적은 **굵기를 눈으로 보는 것**인데 파일럿엔 그게 통째로 빠져 있었다. 숫자만으로는 알아채지 못한다.

## 수정

- 원 지름 = **브러시 크기 × 화면 표시 배율**. 브러시 크기는 소스 픽셀 단위이므로(획 표본이
  `mapClientPointToSource`로 소스 좌표에 매핑된다) 배율 보정이 필수다. 레거시의
  `rect.width / canvas.width`에 대응하되, 확대·이동이 반영된 `resolveEffectiveCanvasRect`를 재사용한다.
- 채움 `#RRGGBB80`, 테두리는 브러시 색. `style.color`는 스키마상 항상 6자리 hex라 8자리 알파 표기가 안전하다.
- 앵커 좌표가 원의 **중심**이 되므로 `SIZE_ADJUST_HUD_OFFSET_X/Y` 상수는 필요 없어져 제거했다.
- 라벨을 원의 자식 `span`으로 분리했다. 원의 `width`/`height`가 매 프레임 바뀌는데 글자는 그대로여야 한다.

## "화면 중앙"의 정체

레거시 `updateBrushSizeHud`가 좌표 없는 호출에서 화면 중앙으로 폴백한다.

```js
const x = Number.isFinite(detail.clientX) ? detail.clientX : window.innerWidth / 2;
```

그런데 Alt 드래그는 `_emit('sizeadjust', { clientX: this._sizeAdjustStartX, ... })`로 **드래그 시작 좌표를 보낸다.**
즉 레거시에서도 Alt 드래그 HUD는 시작점에 뜬다. **화면 중앙은 `[` / `]` 단축키·슬라이더 경로의 폴백이다.**

`[` / `]`는 아직 IPC 채널조차 없어 작업 5에서 신설하므로, 그때 중앙 HUD를 함께 띄우도록
계획서 §5.7 (d-5)에 설계를 남겼다(자동 숨김 타이머 + Alt 제스처 시 타이머 취소 포함).

## 결과

- 신규 테스트 1건(배율 0.5 세션에서 크기 3 → 최소 2px, 13 → 7px, 채움 `#ff475780`, 테두리 `#ff4757`)
- 기존 Alt HUD 단언 갱신 — 앵커 오프셋 제거(`116px` → `100px`), 라벨 자식 분리
- 25스위트 **2,172 → 2,173 pass / 0 fail**

## 배포 기록 (v2.4.5-beta)

| 항목 | 값 |
|------|-----|
| 배포 시각 | **2026-08-28 18:43** |
| 소스 커밋 | `c17c0e3` (main, PR #191 머지) |
| 배포 방식 | `robocopy /MIR` — exit 1(정상), FAILED 0 |
| 검증 | 전체 트리 SHA-256 **82/82, MismatchCount=0 / MissingCount=0 / ExtraFiles=0** |
| exe SHA-256 | `2C723C5FA5A53DF50C5EB46D531840B9BED1766AA8BF033A4628DDE74DE44AED` |
| app.asar SHA-256 | `1C96E7D16B75C548C10039175F6CFB04D9C8CDA536FB3FD60E71BA7FECC47E3A` |
| `FileVersion` | `2.4.5-beta` |
| 직전 배포본 | v2.4.4-beta, 2026-08-28 18:27 (교체됨) |

**실기 확인**: `B` → Alt 드래그 → 브러시 색 원이 드래그 시작점에 뜨고 크기에 따라 커졌다 작아진다. 위에 `NNpx` 라벨.

---

# 작업 2 — Ctrl+Z 전역 실행취소 (PR ②, v2.4.6-beta)

## 문제

히스토리가 키프레임(씬)별로 분리되어 `moveHistory()`가 `activeScene()` 하나만 봤다.
재생헤드가 키프레임에 정확히 있지 않으면 되돌릴 대상이 없어 **아무 반응이 없었다.**

## 설계 — 기존 구조를 갈아엎지 않았다

계획 문서 §6.4는 "씬 단위 예산 회계 재설계 + `undoDepth` 단언 116줄 흔들림"을 우려했지만
**그 재설계는 필요 없었다.** 아래를 전부 그대로 뒀다.

- `scene.history` 인스턴스와 `scene.historyEntries` 바이트 거울 (씬별)
- `estimateSceneBytes` / `reachableObjectBytes` / `planProjectedEvictions` (씬 단위 예산 회계)
- `undoDepth` / `redoDepth` 진단의 의미 = **활성 씬의 깊이**

새로 만든 것은 `Map<stableVideoIdentity, { undo, redo }>` 하나뿐이고 항목은 `{ sceneKey, commandId }`다.
전역 깊이는 `globalUndoDepth` / `globalRedoDepth`라는 **새 필드**로 낸다.

### 정합성이 성립하는 이유

한 씬 안에서 커맨드는 `scene.history`의 `undoStack`과 `order.undo`에 **같은 순서로** 쌓인다.
따라서 `order.undo`의 마지막 항목은 언제나 그 씬 히스토리의 최상단이다. 전역 undo는
"최상단 항목의 씬을 골라 그 씬의 `history.undo()`를 호출"하면 끝이고, **씬 히스토리
자료구조를 건드릴 필요가 없다.**

영상별로 나눈 이유: 실행취소는 문서 단위여야 하고, 씬 축출(`evictVideo`)이 영상 단위라
인덱스 회수 경계가 자연히 일치해 유령 항목이 생기지 않는다.

## 구현하며 새로 막은 결함 — 전역화가 만드는 고착

`applyHistoryState`는 실패할 수 있고(`scene-capacity-exceeded`·`mutation-sequence-overflow` 등)
그때 `moveTop`은 스택을 pop하지 않는다. **최상단에서 멈추면 그 항목이 영구히 남아
그 시점부터 모든 Ctrl+Z가 같은 실패를 반복한다.** 씬별 히스토리에는 없던 문제다 —
예전에는 다른 키프레임으로 옮기면 그 씬의 undo가 동작했다.

후보를 순회하며 실패 항목은 **버리지 않고 건너뛴다**(`globalHistoryCandidates`, 최대 8회).
일시적 실패라면 다음에 다시 시도할 수 있어야 하기 때문이다. 건너뛰어도 불변식은 유지된다 —
각 항목은 여전히 자기 씬 히스토리의 최상단이고 상대 순서도 그대로다.

## 재생헤드는 옮기지 않는다 (결정 5)

다른 키프레임을 되돌리면 화면에 변화가 없다. 이때 캔버스를 다시 그리고 도구 모드를
재설정하면 헛일이고 사용자의 활성 선택만 사라지므로, `affectedActiveScene`으로 활성 씬이
실제로 바뀐 경우에만 손댄다.

`notifyCommittedTransition`은 되돌린 씬 기준으로 그대로 발화하므로 **화면에 안 보여도
저장은 된다.** 테스트가 이 계약을 고정한다.

## 기존 테스트 2건 — 단언을 완화하지 않고 의도를 분리했다

| 테스트 | 조치 |
|---|---|
| `video and frame scene histories remain independent across undo and redo` | **영상 간 독립성**은 여전히 유효하므로 지키고, **프레임 간 독립성**은 전역화가 의도적으로 바꾸는 성질이라 시각 순서 의미를 명시하도록 재작성. "재생헤드를 25에 둔 채 redo하면 24가 복원되고 화면(25)엔 변화가 없다"까지 단언해 오히려 강해졌다 |
| `later commands on sibling frames cannot consume the capacity reserved for an admitted undo redo` | 본래 의도는 **용량 예약**이므로 그대로 두고, 전역 순서상 형제 프레임 항목을 먼저 걷어낸 뒤 프레임 0의 항목에 닿도록 변경 |

## 계획서와 달랐던 지점

계획서는 `record()` **실패 경로에도** `clearedRedoIds`를 실으라고 했다. 방어로서는 옳지만,
커맨드가 통째로 용량을 넘는 경우는 `clearStack(redoStack)` **이전**에 조기 반환하므로
그 경로는 실제로 도달할 수 없다. 방어 코드는 두되 테스트는 성립하는 불변식
("조기 반환은 redo를 폐기하지 않는다")으로 바꿨다.

## 작업 중 걸린 것 — Python 쓰기의 CRLF 변환

`io.open(p, 'w')`가 Windows에서 개행을 CRLF로 바꿔 `test:mpv`의 소스 정규식 테스트가 깨졌다
(`
    };

    let collabRippleAnimationId` 패턴이 매칭되지 않음).
저장소 규약은 `.gitattributes`의 `* text=auto`(LF)이고 커밋본은 정상이었으니 작업 사본만
오염된 것이었다. LF로 되돌리고 이후 쓰기는 `newline=''`로 고정했다.
**교훈: 이 저장소에서 Python으로 파일을 쓸 때는 반드시 `newline=''`를 준다.**

## 결과

- 신규 12건 — 런타임 8(재생헤드가 키프레임에 없을 때 동작·비활성 씬의 저장 전파·적용 실패 시 고착 방지 포함), 명령 히스토리 3, 호스트 1
- 25스위트 **2,173 → 2,185 pass / 0 fail**
- 기존 `undoDepth` 참조 116줄 **무영향**

## 배포 기록 (v2.4.6-beta)

| 항목 | 값 |
|------|-----|
| 배포 시각 | **2026-08-28 19:28** |
| 소스 커밋 | `afa2663` (main, PR #194 머지) |
| 배포 방식 | `robocopy /MIR` — exit 1(정상), FAILED 0 |
| 검증 | 전체 트리 SHA-256 **82/82, MismatchCount=0 / MissingCount=0 / ExtraFiles=0** |
| exe SHA-256 | `A1AD796F722860C48FA1441922650F4E77F09A9E6B18D73CDD1121B2390F0422` |
| app.asar SHA-256 | `2484FB7767F0702041E8B4CA761D89D668268BE0335BEDE730D2C20D408BD039` |
| `FileVersion` | `2.4.6-beta` |
| 직전 배포본 | v2.4.5-beta, 2026-08-28 18:43 (교체됨) |

### 실기 확인 필요 (부록 A의 A-6 ~ A-12)

1. 프레임 10에 획 → 20에 획 → 20에서 `Ctrl+Z` → 20의 획만 사라진다
2. 이어서 `Ctrl+Z` → **10의 획이 사라진다.** 재생헤드는 20 그대로, 화면 변화 없음 (정상)
3. 프레임 15(키프레임 아님)로 이동 후 `Ctrl+Z` → **가장 최근 편집이 되돌려진다.** 무반응이 아니다
4. 2번 뒤 `Ctrl+Y` 2회 → 10·20이 순서대로 복원
5. 2번 뒤 프레임 30에 새 획 → `Ctrl+Y` → 아무 일도 일어나지 않는다
6. 2번 뒤 저장하고 파일 재오픈 → **되돌려진 상태가 저장돼 있다**
7. 다른 영상을 열고 `Ctrl+Z` → 앞 영상의 편집이 되돌려지지 않는다

---

# 작업 2-b — 키프레임 생성도 실행취소 대상으로 (PR ②b, v2.4.7-beta)

## 사용자 보고

v2.4.6-beta 실기: "프레임 10에 획 → 20에 획 → `Ctrl+Z` 두 번"에서 **두 번째가 먹지 않는다.**

판별 실험 두 개로 범위를 좁혔다.
- 같은 프레임에 획 2개 → `Ctrl+Z` 두 번: **둘 다 정상** → 전역 스택과 키 릴레이는 문제없음
- 실패 상태에서 프레임 10으로 옮겨 `Ctrl+Z`: **아무 일도 없음**

## 원인 — 되돌릴 수 없는 절반

**제 인덱스 버그가 아니었다.** 재현 스크립트에서 전역 인덱스는 정확히 프레임 10을 겨냥했고
10의 획도 지워졌다. 문제는 **키프레임 20이 이미 10의 복사본을 들고 있었다**는 것이다.

키프레임은 직전 키프레임 내용을 복사해 만들어진다(홀드 모델). 사용자의 실사용 `.bframe`이
그 증거다 — 키프레임별 객체 수가 **0, 2, 3, 4, 6, 8, 8, 11, 12, 14, 16, 16으로 단조 증가**한다.

그런데 그 복사(`materializeProvisionalScene`)는 지속화에만 알리고 **히스토리 커맨드를 남기지 않았다.**
즉 "프레임 20으로 옮겨 그리기"라는 한 동작이 실제로는 둘이었다.

| | 실행취소 대상 |
|---|---|
| 1. 키프레임 20을 10의 복사본으로 생성 | ❌ |
| 2. 획 B 추가 | ✅ |

**전역 실행취소가 만든 버그가 아니라 전역화가 드러낸 의미 충돌이다.** 예전에는 활성 키프레임만
되돌릴 수 있어서 이 모순을 볼 일이 없었다.

## 수정

### 1. 키프레임 생성을 같은 커맨드에 묶는다

정식화 커맨드에 `materializesScene` 표식을 남기고, 되돌릴 때 씬을 다시 `provisional`로 되돌린다.
삭제하지 않는 이유는 redo가 그 씬을 다시 찾아야 하기 때문이며, `provisional` 씬은
`exportVideo`(`:1482`)와 `resolveCommittedSceneAtFrame`(`:2070`)이 **이미 둘 다 건너뛴다.**
파일과 이후 프레임 해석에서는 키프레임이 사라진 것과 같아 **새 지속화 삭제 경로가 필요 없다.**

### 2. 임시 씬의 화면은 커밋된 원본에서 파생한다

씬 객체 안에는 만들어질 당시의 사본이 남아 있어 원본이 되돌려지면 낡는다.
처음에는 사본을 그 자리에서 갈아끼웠는데 **그 씬의 redo가 `invalid-history-state`로 죽었다** —
히스토리가 절대 스냅샷이라 객체를 바꾸면 자기 상태와 어긋난다.

그래서 **저장된 사본은 불변으로 두고 `getActiveSceneSnapshot`이 화면용으로만 다시 파생**한다.
낡은 사본 위에 커밋하면 지워진 획이 되살아나므로 변경 진입점 5곳
(`addStroke`/`transformSelection`/`replaceObjects`/`deleteSelection`/`clearSession`)에서
커밋 전에 원본으로 맞춘다. 그때 그 씬의 redo를 비우는 것은 "되돌린 뒤 새 동작을 하면
다시실행이 사라진다"는 일반 규칙과 같다.

**두 수정 모두 "되돌려서 임시가 된 씬"에만 적용한다.** 한 번도 편집되지 않은 스크래치 임시 씬의
내용은 `activateSession`이 정한 것이고 낡음 문제가 없다. 범위를 좁히지 않았을 때 관계없는
기존 테스트 3건이 깨져 이를 확인했다.

### 3. 되돌린 씬을 프레임 이동 때 버리지 않는다

임시 씬은 원래 자리를 뜨면 버려졌다. 되돌려서 임시가 된 씬은 redo용 히스토리를 들고 있어,
그것까지 버리면 **되돌린 뒤 프레임을 옮기는 순간 다시 실행이 영영 불가능해진다.**

## 검증한 성질

| 동작 | 화면 | 저장 파일 |
|---|---|---|
| 그린 직후 | A, B | `10:[A]`, `20:[A,B]` |
| `Ctrl+Z` 1회 | A | `10:[A]` |
| `Ctrl+Z` 2회 | **비어 있음** | **비어 있음** |
| `Ctrl+Y` 2회 | A, B | `10:[A]`, `20:[A,B]` |
| 되돌린 뒤 새로 그림 | C | `20:[C]` — 지운 A가 안 살아남 |

## 이 결함이 작업 2의 테스트를 빠져나간 이유

작업 2의 테스트는 `activateSession`으로 프레임을 바꿨는데, **실기의 프레임 이동은
`retargetActiveSession`이고 복사는 그 경로에서만 일어난다.** 같은 경로를 타는 회귀 테스트를 추가했다.

## 의도된 동작 변경 — 기존 테스트 2건

`held frame first stroke materializes the target while preserving the source and undo base`와
`toolbar clear undo and redo retarget the armed frame...`가 **"첫 획을 되돌려도 키프레임이
held base와 함께 남는다"**를 계약으로 못박고 있었다. 그 계약이 위 모순의 근원이며 사용자가
애니메이트 동치로 가기로 결정했다. 단언을 완화하지 않고 새 계약과 redo 복원을 함께 단언한다.

## 과정 기록 — 설계 재검토 1회

수정이 세 번 연달아 다른 곳에서 문제를 드러냈다(사본 갱신 → redo 깨짐 → 범위 축소 → 화면 잔상 잔존).
그 시점에서 멈추고 사용자에게 선택지 3개를 제시했다. "저장된 사본은 불변, 화면만 파생"으로
방향을 바꾸고 나서야 네 성질이 동시에 성립했다.

## 절차 위반 1건

작업 도중 **`main`에 직접 커밋했다.** PR ② 머지 시 `--delete-branch`로 브랜치가 사라지면서
`main`에 남아 있는 것을 놓쳤다. 발견 즉시 커밋을 새 브랜치로 옮기고 `main`을 `origin/main`으로
되돌린 뒤 PR로 재상신했다 — **원격 `main`에는 직접 커밋이 올라가지 않았다.**
재발 방지: 머지 직후 다음 작업 브랜치를 먼저 만든다.

## 결과

- 신규 3건, 기존 갱신 5건. 25스위트 **2,185 → 2,188 pass / 0 fail**

## 배포 기록 (v2.4.7-beta)

| 항목 | 값 |
|------|-----|
| 배포 시각 | **2026-08-28 22:43** |
| 소스 커밋 | `304f68c` (main, PR #197 머지) |
| 배포 방식 | `robocopy /MIR` — exit 1(정상), FAILED 0 |
| 검증 | 전체 트리 SHA-256 **82/82, MismatchCount=0 / MissingCount=0 / ExtraFiles=0** |
| exe SHA-256 | `06FC2B0AAA6E064EB9F61274AA8CFEA5A05919E3BD17C5E47A6A6F0FA57E545F` |
| app.asar SHA-256 | `52B73E2B7C12A5821951C6D2B8959CE174DED8FFC4A09D2176C2A8353A65326D` |
| `FileVersion` | `2.4.7-beta` |
| 직전 배포본 | v2.4.6-beta, 2026-08-28 19:28 (교체됨) |

**실기 확인**: 프레임 10에 획 → 20으로 옮겨 획 → `Ctrl+Z` 두 번 →
20에 서 있어도 화면에서 획이 전부 사라진다. `Ctrl+Y` 두 번으로 완전 복원.
