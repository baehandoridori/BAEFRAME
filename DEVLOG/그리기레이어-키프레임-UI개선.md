...
| 작업 | 상세 |
|------|------|
| `_renderLayerTrack` 수정 | 범위 블록 + 키프레임 셀 분리 렌더링 |
| `_createKeyframeCell` 추가 | 고정 위치 키프레임 마커 생성 |
| CSS 스타일 추가 | `.keyframe-cell`, `.keyframe-dot`, `.keyframe-range-block` |
| 프레임 번호 표시 | 줌 200% 이상에서 자동 표시 |

### 구현된 파일 변경

| 파일 | 변경 내용 |
|------|----------|
| `timeline.js` | `_renderLayerTrack`, `_createKeyframeCell` 수정/추가 (~75줄) |
| `main.css` | 키프레임 셀 스타일 추가 (~95줄) |
