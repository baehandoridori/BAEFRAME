...
> **상태: ⏸️ 보류 (2026-01-26)**
>
> 리스크 대비 효과 검토 결과, 팀 전체 동시 업데이트 필요성과 기존 파일 마이그레이션 복잡도로 인해 보류.
> 추후 팀 규모 확대 또는 파일 관리 이슈가 심각해지면 재검토.

## 요약

| 항목 | 수정 파일 | 이유 | 우선순위 | 상태 |
|------|----------|------|---------|------|
| Phase 1 | preload.js, ipc-handlers.js | 누락된 파일 API 추가 (copyFile, deleteFile, scanTempFiles) | 높음 | ⏸️ 보류 |
| Phase 2 | file-cleanup-manager.js (신규) | 파일 정리 매니저 모듈 생성 | 높음 | ⏸️ 보류 |
| Phase 3 | review-data-manager.js | 마이그레이션 후 백업 자동 삭제 로직 연결 | 중간 | ⏸️ 보류 |
| Phase 4 | settings.html, settings.js | 설정 UI에 정리 옵션 추가 | 중간 | ⏸️ 보류 |
| Phase 5 | main.js | 앱 시작 시 자동 정리 훅 | 낮음 | ⏸️ 보류 |
