...
|-------|----------|------|---------|------|
| Phase 1 | main/recent-files-store.js, scripts/tests/recent-files-store.test.js | 저장소 로직과 단위 테스트 | 높음 | ✅ 완료 |
| Phase 2 | main/recent-thumb-capture.js, main/ipc-handlers.js, main/index.js, preload/preload.js | ffmpeg 썸네일 캡처 및 IPC 노출 | 높음 | ✅ 완료 |
| Phase 3 | renderer/scripts/modules/recent-files-manager.js, renderer/scripts/modules/recent-files-view.js, renderer/styles/recent-files.css, renderer/styles/main.css | 렌더러 모듈과 스타일 | 높음 | ✅ 완료 |
| Phase 4 | renderer/index.html, renderer/scripts/app.js | 기존 UI에 통합 | 높음 | ✅ 완료 |
| Phase 5 | (수동 QA) | 10개 수동 테스트 시나리오 통과 | 높음 | 🔄 진행중 |
...
- [ ] 1. 기본 추가: 새 영상 파일 열기 → 앱 재시작 → 목록에 해당 항목 보임
- [ ] 2. 중복 열기: 같은 파일 여러 번 열기 → 하나의 항목만 존재
- [ ] 3. 용량 트림: 11개째 비고정 항목 열기 → 가장 오래된 비고정 사라짐
- [ ] 4. 고정: 임의 항목 고정 → 재시작 후에도 상단 유지
- [ ] 5. 고정 한계: 6번째 고정 시도 → 토스트 표시
- [ ] 6. 파일 이동/삭제: 앱 재시작 시 자동 정리 또는 회색 처리
- [ ] 7. 전체 지우기: 버튼 클릭 → 섹션 전체 사라짐
- [ ] 8. 헤더 드롭다운: 시계 버튼 클릭 → 패널 표시/Esc 닫힘
- [ ] 9. 드래그앤드롭 호환: 리스트 영역에서 파일 드롭 → 정상 로드
- [ ] 10. 오디오 파일: mp3/wav 열기 → music 아이콘 표시

수동 QA 미완료 — 사용자가 `npm run dev`로 앱 실행 후 확인 필요.
