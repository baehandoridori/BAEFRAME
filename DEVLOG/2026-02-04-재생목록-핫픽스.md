...
await this._generatePhase2(signal);
if (signal.aborted) {
  // Phase 2 중단됨 → 현재까지 생성된 썸네일로 캐시 업데이트
  await this._saveToCache();
  log.info('Phase 2 중단, 현재까지의 캐시 저장', { count: this.thumbnailMap.size });
  return;
}

// Phase 2 완료 시 최종 캐시 저장
await this._saveToCache();
```

**핵심 변경점**:
- Phase 1 완료 직후 `_saveToCache()` 호출 추가 (line 148 이후)
- Phase 2 abort 시에도 현재까지의 썸네일을 캐시에 저장
- 다음에 같은 파일을 열면 캐시에서 즉시 로드 (오버레이 최소화)
...
| `renderer/scripts/modules/thumbnail-generator.js` | Phase 1 완료 후 `_saveToCache()` 추가, Phase 2 abort 시에도 캐시 저장 | 3 |
