...
| 항목 | 수정 파일 | 이유 | 우선순위 | 상태 |
|------|----------|------|---------|------|
| Phase 1: EBUSY 변환 실패 수정 | `main/ffmpeg-manager.js` | rename 시 파일 핸들 미해제로 EBUSY 오류 | 높음 | ✅ 완료 |
| Phase 2: 재생목록 데이터 소실 수정 | `renderer/scripts/modules/playlist-manager.js`, `renderer/scripts/app.js` | 미저장 재생목록 데이터 소실 | 높음 | ✅ 완료 |
| Phase 3: 썸네일 캐시 미저장 수정 | `renderer/scripts/modules/thumbnail-generator.js` | Phase 2 abort 시 캐시 저장 누락 | 높음 | ✅ 완료 |
| Phase 4: 재생목록 사전 변환 구현 | `main/ffmpeg-manager.js`, `main/ipc-handlers.js`, `preload/preload.js`, `renderer/scripts/app.js` | 다음 영상 사전 변환으로 UX 개선 | 중간 | ✅ 완료 |
