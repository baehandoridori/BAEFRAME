...
|-------|------|----------|------|---------|------|
| 1 | 스키마 및 데이터 구조 | shared/schema.js, new: playlist-schema.js | .bplaylist 파일 포맷 정의 | 높음 | ✅ 완료 |
| 2 | 썸네일 캐시 시스템 | main/ipc-handlers.js, review-data-manager.js | 영상 열 때 자동 썸네일 생성 | 높음 | ✅ 완료 |
| 3 | 기본 UI (좌측 사이드바) | index.html, main.css, new: playlist-panel.css | 재생목록 패널 기본 구조 | 높음 | ✅ 완료 |
| 4 | 재생목록 매니저 모듈 | new: playlist-manager.js | 재생목록 CRUD, 파일 I/O | 높음 | ✅ 완료 |
| 5 | 파일 추가/제거 기능 | playlist-manager.js, app.js | 드래그 앤 드롭, 파일 선택 | 높음 | ✅ 완료 |
| 6 | 재생 네비게이션 | playlist-manager.js, app.js | 이전/다음 영상, 자동 재생 | 높음 | ✅ 완료 |
| 7 | 피드백 완료율 표시 | playlist-manager.js, playlist-panel.css | resolved 댓글 비율 계산 | 중간 | ✅ 완료 |
| 8 | 드래그 순서 변경 | playlist-manager.js | 리스트 재정렬 | 중간 | ✅ 완료 |
| 9 | 링크 공유 기능 | main/index.js, ipc-handlers.js, app.js | baeframe://playlist 프로토콜 | 높음 | ✅ 완료 |
...
> 새 세션에서 구현 시 혼란될 수 있는 부분들을 Q&A로 정리

---

### Q1. 재생목록 UI는 어디서 시작하나요?

**A:** 헤더에 **"재생목록" 버튼**을 새로 추가합니다.

```html
<!-- index.html 헤더 header-right 영역, btnSave 앞에 추가 -->
<button class="btn btn-secondary" id="btnPlaylist" title="재생목록">
  <svg><!-- 리스트 아이콘 --></svg>
  재생목록
</button>
```
...
**동작:**
- 버튼 클릭 → 좌측 사이드바 토글 (열기/닫기)
- 재생목록이 없으면 → 새 재생목록 자동 생성
- 재생목록이 있으면 → 기존 재생목록 표시

---

### Q2. 재생목록은 언제 저장되나요?

**A:** **자동 저장** 방식 (기존 .bframe과 동일)

| 시점 | 동작 |
|------|------|
| 파일 추가/제거 시 | 즉시 저장 (파일이 있으면) |
| 이름 변경 시 | 즉시 저장 |
| 순서 변경 시 | 즉시 저장 |
| 앱 종료 시 | 자동 저장 |
| 링크 복사 시 | 저장 안 됐으면 먼저 저장 |

**단, 첫 저장은 수동:**
- 새 재생목록 → 파일 1개 이상 추가 → 링크 복사 또는 닫기 시 저장 경로 결정

---

### Q3. 기존 영상 열기와 재생목록의 관계는?

**A:** **독립적으로 동작**

| 시나리오 | 재생목록 상태 |
|---------|-------------|
| 앱 시작 → 영상 드롭 | 재생목록 없음 (단독 영상 모드) |
| 재생목록 버튼 클릭 | 빈 재생목록 생성, 현재 영상 자동 추가 옵션 |
| 재생목록 링크로 열기 | 재생목록 모드 |
| 재생목록 닫기 | 현재 영상 유지, 단독 모드로 전환 |

**중요:** 재생목록 없이도 앱은 정상 동작해야 함 (기존 기능 유지)

---

### Q4. 한 번에 여러 재생목록을 열 수 있나요?

**A:** **아니요, 하나만**

- 새 재생목록 열면 → 기존 재생목록 자동 저장 후 닫힘
- 재생목록 간 전환은 별도 UI 없음 (링크로만)

---

### Q5. 앱 시작 시 마지막 재생목록을 기억하나요?

**A:** **아니요** (V1 범위)

- 앱 시작 → 항상 재생목록 없는 상태
- 재생목록을 다시 열려면 → 링크 클릭 또는 .bplaylist 파일 열기
- 향후 개선 가능: 설정에서 "마지막 재생목록 기억" 옵션

---

### Q6. 썸네일 캐시 키는 어떻게 생성하나요?

**A:** **파일 경로의 MD5 해시** (기존 타임라인 썸네일과 동일)
...
// 결과: "a1b2c3d4e5f6g7h8"
// 저장 경로: %APPDATA%/baeframe/thumbnails/a1b2c3d4e5f6g7h8_thumb.jpg
```

---

### Q7. loadVideo 함수는 어떻게 호출하나요?

**A:** 기존 함수 시그니처 사용 (`app.js:2921`)
...
// .bframe 파일은 자동으로 처리됨:
// 1. loadVideo가 videoPath에서 .bframe 경로 추론
// 2. reviewDataManager가 .bframe 파일 로드
// 3. 없으면 새로 생성
```

**중요:** `item.bframePath`를 별도로 전달할 필요 없음. reviewDataManager가 알아서 처리.

---

### Q8. 에러 UI는 어떻게 표시하나요?

**A:** 기존 `showToast()` 함수 사용
...
// 에러
showToast('파일을 찾을 수 없습니다: shot_001.mp4', 'error');

// 경고
showToast('최대 50개까지 추가할 수 있습니다.', 'warning');

// 정보
showToast('3개 파일이 추가되었습니다.', 'info');
```
...
1. **재생목록 패널 헤더**에 삭제 버튼 (휴지통 아이콘)
2. 클릭 → 확인 다이얼로그 → .bplaylist 파일 삭제
3. 재생목록 닫힘, 단독 영상 모드로 전환

```javascript
async function deletePlaylist() {
  const confirmed = await showConfirmDialog('재생목록을 삭제하시겠습니까?');
  if (!confirmed) return;

  await window.electronAPI.deleteFile(playlistManager.playlistPath);
  playlistManager.close();
  hidePlaylistSidebar();
  showToast('재생목록이 삭제되었습니다.', 'info');
}
```

---

### Q10. autoPlay, floatingMode 설정은 어디에 저장되나요?

**A:** **두 군데에 저장**

1. **.bplaylist 파일 내부** (재생목록별 설정)
   ```json
   {
     "settings": {
       "autoPlay": true,
...
2. **앱 전역 설정** (electron-store) - 기본값으로 사용
   ```javascript
   // 재생목록 열 때
   const globalSettings = await window.electronAPI.loadSettings();
   const defaultAutoPlay = globalSettings.playlist?.autoPlay ?? false;
   ```

**우선순위:** 재생목록 파일 설정 > 전역 설정 > 기본값(false)

---

### Q11. .bframe 없는 영상을 재생목록에 추가하면?

**A:** **정상 추가, .bframe은 영상 로드 시 자동 생성**

```javascript
// 재생목록 추가 시
const item = {
  videoPath: 'G:/프로젝트/shot_001.mp4',
  bframePath: '',  // 아직 없음
  // ...
};
...
// 해당 영상 선택 시
await loadVideo(item.videoPath);
// → reviewDataManager가 shot_001.bframe 자동 생성
// → 이후 피드백 가능
```

---

### Q12. 재생목록 안의 영상 경로가 바뀌면?
...
async function loadVideoFromPlaylist(item) {
  ...
  if (!exists) {
    ...
    // UI에서 해당 아이템에 "파일 없음" 표시
    markItemAsMissing(item.id);
    return false;
  }
...
**UI 표시:**
```css
.playlist-item.missing {
  opacity: 0.5;
}
.playlist-item.missing::after {
  content: '⚠️ 파일 없음';
}
```
...
### Q13. 재생목록 열기 진입점은 몇 가지인가요?

**A:** **3가지**

| 진입점 | 동작 |
|--------|------|
| 1. 헤더 "재생목록" 버튼 | 새 재생목록 또는 현재 재생목록 토글 |
| 2. baeframe://playlist 링크 | .bplaylist 파일 로드 후 표시 |
| 3. .bplaylist 파일 드래그앤드롭 | 기존 dropZone에서 감지 후 재생목록 모드 |

**3번 구현:**
```javascript
// app.js dropZone drop 이벤트 수정
elements.dropZone.addEventListener('drop', async (e) => {
  const file = e.dataTransfer.files[0];

  if (file.path.endsWith('.bplaylist')) {
    // 재생목록 파일
    await playlistManager.open(file.path);
    showPlaylistSidebar();
  } else if (file.path.endsWith('.bframe')) {
    // .bframe 파일 → 연결된 영상 로드
    // 기존 로직
  } else {
    // 영상 파일
...
**화면이 좁을 때:** 재생목록 패널 자동 숨김 (미니 인디케이터만 표시)

---

### Q15. 구현 시작 전 확인 사항

새 세션에서 구현 시작 전 **필수 확인:**

```bash
# 1. 브랜치 확인
git branch  # claude/plan-playlist-feature-... 에서 작업

# 2. 기존 코드 구조 확인
ls renderer/scripts/modules/  # 기존 모듈 패턴 확인

# 3. ffmpeg 존재 확인
ls mpv/ffmpeg.exe  # 썸네일 생성에 필요

# 4. 기존 드롭존 동작 확인
npm run dev  # 영상 드래그앤드롭 테스트
```

**첫 작업:** Phase 1 (스키마) → Phase 3 (UI) → Phase 4 (매니저) 순서 권장
- 스키마 먼저: 데이터 구조 명확화
- UI 다음: 시각적으로 확인하면서 개발
- 매니저: UI와 연결하며 개발
- 전체화면에서 재생목록 네비게이션
