...
- 추가 릴리스 검사: 저장·복구 164, 재생목록 330, 드로잉 334 모두 exit 0 / fail 0 / cancelled 0. 재생목록의 기존 source 추출기 2곳이 메인 document에 고정되어 최초 실행에서 2개 실패했다. 실제 동작 assertion은 유지하고 현재 패널 조회를 추출하도록 보강한 뒤 330개 전부 통과했다. [추가 검사 결과](evidence/2026-09-07-comment-popout-release/extra-tests.json).
...
- `db254b5` 리뷰의 P2 두 건을 실제 코드에서 확인해 수정했다.
- `main/editor-media.js`: OS가 프로세스 종료 시 해제하는 잠금과 hostname/PID/token을 함께 사용한다. 같은 PC에서 종료된 소유자의 잠금만 회수하며, 새 소유자로 바뀐 잠금은 게시/정리 직전에 다시 확인한다. 다른 PC·소유자 불명·구버전 PID-only 잠금은 추정해서 삭제하지 않고 다른 이름 저장과 소유자 확인을 안내한다.
- `controller.js`: revision 번호 대신 실제 저장된 내용과 현재 내용을 비교한다. 저장 중 추가된 그림은 미저장으로 유지하고, undo/redo로 실제 저장점에 돌아오면 미저장 표시와 닫기 확인을 해제한다. 직렬화·비교는 문서 변경/저장 시에만 실행해 재생 프레임마다 큰 그림 데이터를 다시 비교하지 않는다.
- 재현 후 수정: editor 전체 **147 pass / 0 fail / 0 cancelled / exit 0**, UX **70 pass / 0 fail / 0 cancelled / exit 0**, 변경 파일 lint 오류/경고 0.
- 실제 Electron 편집기 **11개 통과**: 기존 영상 만들기 10개에 저장점으로 undo 후 불필요한 확인 없이 정상 닫기까지 추가했다. 잠금은 실제 child 강제 종료 후 같은 `.bedit` 저장·실제 FFmpeg MP4 재출력, 살아 있는 소유자·외부 PC 보호, 동시 복구자 6개 중 한 번만 게시, 교체된 잠금 보존을 검사했다.
- 머지 전 배포 후보의 댓글 **17개**(밝은/어두운 테마 포함)와 편집기 **10개**도 통과했다. 이 후보의 package/lock/asar/EXE 버전 일치, stable 프로필, 코드 126개 및 필수 런타임 7개 동등성을 확인했다. 최종 배포는 이후 merge SHA에서 다시 빌드한다.
