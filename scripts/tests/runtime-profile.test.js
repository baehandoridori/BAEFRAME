...
test('stable engine promotion uses the 2.0.2 beta release version consistently', () => {
  const rootDir = path.resolve(__dirname, '..', '..');
  const packageJson = JSON.parse(fs.readFileSync(path.join(rootDir, 'package.json'), 'utf8'));
  const packageLock = JSON.parse(fs.readFileSync(path.join(rootDir, 'package-lock.json'), 'utf8'));

  assert.equal(packageJson.version, '2.0.2-beta');
  assert.equal(packageLock.version, '2.0.2-beta');
  assert.equal(packageLock.packages[''].version, '2.0.2-beta');
});
