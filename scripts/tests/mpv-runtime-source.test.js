...
test('mpv external playback preserves frame seek and loop behavior', () => {
  ...
  assert.match(videoPlayerSource, /if \(status\.stopped === true\) \{[\s\S]+await this\.externalControls\?\.stop\?\.\(\);[\s\S]+this\.useHtml5Engine\(\);[\s\S]+this\.isLoaded = false;[\s\S]+this\._emit\('externalstopped', \{ engine: stoppedEngine \}\);[\s\S]+return;[\s\S]+\}/);
