...
| 상태 | 개수 | 비고 |
|:----:|:----:|------|
| `DONE` | 51 | 핵심 기능 + UI/UX + 그리기 도구 + 타임라인 개선 |
| `WIP` | 2 | 웹 뷰어, 문서화 |
...
| 기능 | 상세 |
|------|------|
| **플레이헤드 드래그 자동 스크롤** | 드래그 시 타임라인 끝에 도달하면 자동 스크롤 |
| **그리기 레이어 UI 개선** | 격자무늬 제거, 회색 색상, 위아래 꽉 참 (#72) |
| **줌 플레이헤드 중심** | 휠/슬라이더 줌 시 플레이헤드 기준으로 줌 |
| **로딩 스플래시 개선** | 링크로 앱 열 때 로딩 화면 표시, 작업표시줄 표시 |
| **DrawingCanvas 메모리 누수 수정** | AbortController로 이벤트 리스너 정리 |
| **마커 위치 드래그 이동** | 댓글 마커를 드래그하여 위치 변경 (#72) |
| **마커 위치 자동저장** | 마커 이동 후 자동으로 .bframe 저장 (#70) |
| **마커 툴팁 버그 수정** | 말풍선 표시/숨김, 구간 외 상호작용 차단 |
| video.play() 에러 처리 | Promise catch로 에러 핸들링 |
| wheel passive 옵션 | 타임라인 스크롤 브라우저 경고 제거 |
| 가상 스크롤 기본 구조 | virtualScrollState 추가, 100개 이상 경고 |
...
| 항목 | 상세 | 상태 |
|------|------|:----:|
| ~~격자무늬 제거~~ | 그리기 레이어에 생성되는 별도 격자 패턴 제거 | ✅ |
| ~~투명도 수정~~ | 그리기 레이어 칸을 불투명하게 변경 (25% → 69%) | ✅ |
...
| 3 | ~~DrawingCanvas.destroy() 리스너~~ | AbortController로 정리 | ✅ |
...
**구현 계획:**
- [x] ffmpeg 번들링 또는 시스템 ffmpeg 감지
- [x] 파일 로드 시 코덱 확인
- [x] 미지원 포맷 자동 변환 (H.264 MP4 또는 VP9 WebM)
- [x] 변환 진행률 UI
- [ ] 캐시 관리 (변환된 파일)
