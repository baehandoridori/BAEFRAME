...
| 구분 | 경로 | 실행 파일/명령 | 의미 |
|------|------|---------------|------|
| **개발** | `C:\BAEFRAME\` | `run-baeframe.bat` (기본 실행), `npm run dev` (디버깅) | 소스 코드를 수정하면서 기능 개발/디버깅하는 환경 |
| **개발 빌드** | `C:\BAEFRAME\dist\win-unpacked\` | `BFRAME_alpha_v2.exe` | 빌드된 exe 형태로 동작을 미리 검증하는 QA 환경 |
| **배포** | `G:\공유 드라이브\JBBJ 자료실\한솔이의 두근두근 실험실\BAEFRAME\테스트버전 빌드\` | `BFRAME_alpha_v2.exe` | 팀원이 실제로 사용하는 공용 실행 환경 |

### 왜 3가지 경로로 나누나?

1. **역할 분리**: 개발(코드 수정)과 빌드 검증(exe 테스트), 팀 실사용(배포)을 분리해 혼선을 줄입니다.
2. **검증 단계 확보**: 소스에서 잘 되는 것과 빌드 exe에서 잘 되는 것은 다를 수 있어, `dist\win-unpacked` 검증 단계를 둡니다.
3. **팀 운영 안정성**: 배포 경로는 팀 공용 위치로 고정해, 비개발자도 동일한 실행 파일을 사용하게 합니다.
4. **문제 추적 용이성**: 이슈 발생 시 어느 경로/실행파일에서 발생했는지 빠르게 특정할 수 있습니다.
