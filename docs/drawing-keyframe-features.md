...
>
>   실행취소는 **씬과 레이어 모델을 함께** 되돌려야 한다. 오버레이는 씬만 되돌릴 수 있고 레이어 목록·배정은 렌더러 쪽(`.bframe` 루트)에 있으므로, 조작할 때 모델의 before/after 를 조작의 **짝 id(`commandId`)** 로 기억해 두었다가 되돌림 응답에 실어 온 같은 id 로 함께 되돌린다. 짝을 안 지으면 삭제를 되돌렸을 때 그림만 살아나 기준 레이어로 떨어지고, 이동을 되돌렸을 때 겹침 순서만 돌아오고 레이어 목록은 옮긴 채로 남는다. 빈 레이어를 지우거나 이미 순서가 맞아 **씬이 안 바뀌는 경우**에도 표식(`layer-model-marker`)으로 짝 id 를 남긴다 — 없으면 그 다음 `Ctrl+Z` 가 엉뚱한 앞 커맨드를 되돌린다. 조작은 씬을 통째로 갈아 끼우고 **전이를 내보내지 않으므로** 지속화 재동기를 따로 요청한다(`persistenceResyncRequired`) — 안 하면 렌더러의 수화 문서가 조작 전 상태로 남아 지운 획이 다시 투영된다. 조작 전체(상태 읽기 → 오버레이 왕복 → 모델 반영)는 한 덩어리로 줄을 세운다: 오버레이 요청만 줄을 세우면 두 단축키가 같은 옛 상태를 읽고 나중 것이 앞선 것의 결과를 덮는다.
>
