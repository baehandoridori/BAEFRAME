...
>
>   겹침 순서는 **랭크**로 다룬다. 오버레이는 레이어를 모르고 오브젝트별 정수 하나만 받는다. 타임라인 위쪽(`layers[0]`)이 화면에서도 위여야 하므로 배열 인덱스를 뒤집는다(캔버스는 나중 오브젝트를 위에 그린다). 랭크는 표시·잠금 집합과 **같은 경로로 계속** 밀어 넣는다 — 새 획이 그리는 순간 제 층에 들어가지 않으면, 레이어를 옮겨 맞춰 놓은 순서가 다음 획 하나에 다시 어긋난다
>
