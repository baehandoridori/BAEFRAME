...
>
>   실행취소는 **씬과 레이어 모델을 함께** 되돌려야 한다. 오버레이는 씬만 되돌릴 수 있고 레이어 목록·배정은 렌더러 쪽(`.bframe` 루트)에 있으므로, 조작할 때 모델의 before/after 를 조작의 **짝 id(`commandId`)** 로 기억해 두었다가 되돌림 응답에 실어 온 같은 id 로 함께 되돌린다. 짝을 안 지으면 삭제를 되돌렸을 때 그림만 살아나 기준 레이어로 떨어지고, 이동을 되돌렸을 때 겹침 순서만 돌아오고 레이어 목록은 옮긴 채로 남는다.
>
