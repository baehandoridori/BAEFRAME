...
>
>   **주의: fabric 의 `visible`/`selectable`/`evented` 만으로는 모자란다.** 라쏘(획 단위·부분)와 획 지우개는 히트테스트를 쓰지 않고 **씬 스냅샷을 직접 훑는다.** 이 세 경로와 활성 선택 복원(`refreshSelectionInteractionPolicy` 끝)에서 제한된 id 를 함께 걸러야 한다. 빠뜨리면 보이지도 않는 획이 선택·분할·이동·삭제된다
