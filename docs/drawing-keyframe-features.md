...
>
>   실행취소는 **씬과 레이어 모델을 함께** 되돌려야 한다. 오버레이는 씬만 되돌릴 수 있고 레이어 목록·배정은 렌더러 쪽(`.bframe` 루트)에 있으므로, 조작할 때 모델의 before/after 를 조작의 **짝 id(`commandId`)** 로 기억해 두었다가 되돌림 응답에 실어 온 같은 id 로 함께 되돌린다. 짝을 안 지으면 삭제를 되돌렸을 때 그림만 살아나 기준 레이어로 떨어지고, 이동을 되돌렸을 때 겹침 순서만 돌아오고 레이어 목록은 옮긴 채로 남는다. 빈 레이어를 지우거나 이미 순서가 맞아 **씬이 안 바뀌는 경우**에도 표식(`layer-model-marker`)으로 짝 id 를 남긴다 — 없으면 그 다음 `Ctrl+Z` 가 엉뚱한 앞 커맨드를 되돌린다. 조작 전체(상태 읽기 → 오버레이 왕복 → 모델 반영)는 한 덩어리로 줄을 세운다: 오버레이 요청만 줄을 세우면 두 단축키가 같은 옛 상태를 읽고 나중 것이 앞선 것의 결과를 덮는다.
>
>   겹침 순서는 **랭크**로 다룬다. 오버레이는 레이어를 모르고 오브젝트별 정수 하나만 받는다. 타임라인 위쪽(`layers[0]`)이 화면에서도 위여야 하므로 배열 인덱스를 뒤집는다(캔버스는 나중 오브젝트를 위에 그린다). 랭크는 표시·잠금 집합과 **같은 경로로 계속** 밀어 넣는다 — 새 획이 그리는 순간 제 층에 들어가지 않으면, 레이어를 옮겨 맞춰 놓은 순서가 다음 획 하나에 다시 어긋난다. 이때 새 획의 자리는 `activeLayerRank` 로 따로 보낸다: 랭크 맵에는 **이미 문서에 있는 id** 만 들어 있어 방금 그은 획은 언제나 빠져 있고, 기본 랭크로 떨어뜨리면 활성 레이어가 무엇이든 늘 기준 레이어 자리에 꽂힌다
>
