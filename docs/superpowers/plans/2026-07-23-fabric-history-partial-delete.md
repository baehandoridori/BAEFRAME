...
redo stack bytes remain included in the store-wide maxBytes calculation across scenes
destructive command over maxHistoryBytes is rejected and scene stays byte-identical
undo/redo state application failure after validation injection leaves scene byte-identical and keeps the original stack entry
```
...
`calculateEstimatedBytes()`는 각 scene의 object bytes에 `scene.history.getDiagnostics().historyBytes`를 더한다. Undo로 entry가 redo stack으로 이동해도 store-wide `maxBytes` 계산에서 빠지면 안 된다.
...
`applyHistoryState()`는 현재 `scene.objects`를 직접 고치지 않는다. 새 `Map`, order, selection, estimated bytes를 지역 변수에 완성하고 object ID·용량을 모두 검증한 뒤 한 번에 scene 필드와 교체한다. 검증 또는 적용 callback이 실패하면 기존 Map 참조, 객체 bytes, mutationCount와 history stack을 그대로 유지한다.
...
partial fragments plus fully enclosed stroke move round-trips through Undo and Redo with every ID, transform and z-order intact
...
zero-distance move after `object:moving` restores the original visual, removes every proxy and remains a no-op
...
`object:moving`이 발생했더라도 최종 `dx === 0 && dy === 0`이면 `abortPendingLassoSelection()`으로 원본 Fabric 객체와 z-order를 복원한다. 이 경로는 scene mutation과 history record를 호출하지 않는다.

move/Delete commit 직전에는 pending의 `sessionId`, `inputRevision`, `sourceMutationCount`를 현재 runtime/store 값과 모두 비교한다. 하나라도 다르면 stale transaction으로 간주해 전용 abort하고 scene mutation을 호출하지 않는다.

혼합 이동 command의 touched ID에는 replacement 원본, 생성되는 모든 fragment, 함께 이동한 완전 포함 persisted 획을 모두 넣는다.
...
queued requests capture immutable session/generation/revision/actionId at keydown and cannot retarget after B or media changes
...
host diagnostics expose only bounded numeric undoDepth, redoDepth and historyBytes
```
...
`makeDrawingActionRequest()`는 queue 삽입 시점의 host generation, video generation, input revision, session ID, 고유 action ID를 복사한 plain object를 만든다. 실행을 기다리는 동안 B 토글·프레임·영상이 바뀌어도 request를 다시 만들지 않으며 host의 stale-token 검증으로 거부한다.
...
실행 전후 테스트 영상 옆의 `.bframe`/recovery sidecar 부재를 기록한다. 기존 `.bframe`을 사용하는 추가 무저장 검증에서는 실행 전후 파일 크기, mtime, SHA-256이 모두 같아야 한다.
