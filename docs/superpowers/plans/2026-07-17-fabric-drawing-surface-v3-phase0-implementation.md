...
**Step 4: 의존성과 bundle script 고정**

- `fabric`을 exact `7.4.0` devDependency로 추가한다.
- 기존 `perfect-freehand`는 exact `1.2.3`으로 고정한다.
- `bundle:mpv-fabric-overlay`를 `esbuild renderer/scripts/modules/mpv-fabric-overlay-runtime.js --bundle --format=iife --global-name=BAEFRAMEFabricOverlayBundle --outfile=renderer/scripts/lib/mpv-fabric-overlay.iife.js`로 추가한다.
- `postinstall` 마지막에 새 bundle 명령을 연결한다.
...
Expected: 플래그 테스트 통과, bundle 재생성 후 tracked output 차이 없음.

**Step 6: 커밋**

```powershell
git add main/experiment-flags.js main/index.js package.json package-lock.json renderer/scripts/lib/mpv-fabric-overlay.iife.js scripts/tests/experiment-flags.test.js
...
```powershell
git add renderer/scripts/modules/mpv-fabric-overlay-runtime.js renderer/scripts/modules/fabric-drawing-pilot-metrics.js renderer/scripts/lib/mpv-fabric-overlay.iife.js scripts/tests/mpv-fabric-overlay-runtime.test.js scripts/tests/fabric-drawing-pilot-session.test.js scripts/tests/fabric-drawing-pilot-benchmark.test.mjs package.json
