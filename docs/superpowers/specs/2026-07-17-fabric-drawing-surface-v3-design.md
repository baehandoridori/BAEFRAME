...
창의 초기 상태는 setIgnoreMouseEvents(true)다. B 진입 시 Fabric session 준비가 성공한 뒤 false로 바꾸고, B 종료 시 가장 먼저 true로 되돌린다. B 토글 중 BrowserWindow 생성, loadURL, setBounds, show, hide, moveTop, destroy를 호출하지 않는다.

focusable: false 상태에서 Windows PointerEvent와 펜 압력이 도달하는지를 첫 통합 시험의 선행 조건으로 검증한다. 이 조건이 실패하면 자동으로 focusable: true를 켜지 않는다. 포커스를 얻은 자식 창이 B와 전역 단축키를 잃게 만들 수 있기 때문이다.

MPVOverlayHost bounds는 전체 videoWrapper다. 드로잉 모드에서 playback control 또는 레터박스 영역까지 가로채는지 실제 앱에서 반드시 확인한다. CSS pointer-events만으로 아래 창에 부분 클릭 통과가 된다고 가정하지 않는다. 이 문제가 실제 조작을 막으면 현재 Phase 0은 No-Go로 중단한다. 기존 passive overlay를 계속 클릭 통과로 두고 실제 영상 canvas 사각형만 덮는 별도 DrawingSurfaceHost는 자동 fallback이 아니라 새 설계 승인을 받아야 하는 Phase 0B 대안이다. forward: true는 현재 커서 깜빡임을 재발시킬 수 있으므로 사용하지 않는다.
...
- 같은 generation에서 Fabric IIFE 재주입은 한 번만 허용한다.
