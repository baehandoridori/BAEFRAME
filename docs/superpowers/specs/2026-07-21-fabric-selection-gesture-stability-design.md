...
`pointercancel`, `lostpointercapture`, `window blur`, 입력 비활성, 세션 변경, destroy에서는 현재 제스처를 종료한다. 실제 transform이 확정되지 않은 상태라면 장면 저장소 snapshot을 다시 렌더링해 화면만 되돌린다. 이 취소 경로는 mutation을 만들지 않으며, 다음 입력을 막는 Fabric transform 상태가 남지 않도록 선택 토큰과 `transformStart`를 함께 비운다.

중복 `pointerup`이나 이미 종료된 pointer ID는 무시한다. 캡처 API가 없는 테스트·구형 환경에서는 예외를 내지 않고 기존 이벤트 흐름을 유지한다.
