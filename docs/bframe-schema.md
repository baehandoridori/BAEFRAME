...
v2.0 파일은 선택적으로 `reviewDocumentId`를 가질 수 있습니다. 이 값은 리뷰 파일의 계보를 식별하며, 한 번 생성된 뒤에는 바꾸지 않습니다. 현재 앱이 이해하지 못하는 최상위 필드(예: 미래의 `drawingsV3`)도 열기와 저장 사이에 원문 그대로 보존합니다.

### 이어받은 리뷰 (`reviewCarryoverV1`)

기존 루트 버전 `2.0`을 유지하며, 이전 리뷰 지적을 명시적으로 이어받았을 때 선택 필드
`reviewCarryoverV1: { "version": 1, "items": [...] }`에 이어받은 원문과 현재 영상에서의 미해결·해결 상태를 저장합니다.
현재 `comments`나 이전 영상의 원본 댓글·답글·해결 상태·드로잉을 수정하지 않습니다.

각 항목은 다음 필드를 가집니다.

| 필드 | 의미 |
|------|------|
| `id` | `JSON.stringify([원본 영상의 정규화 경로, 원본 댓글 ID])`. Windows 슬래시와 대소문자를 통일합니다. |
| `source` | 이어받을 당시의 원문 사본. 아래 snapshot 필드를 포함합니다. |
| `status` | 저장 호환성을 위해 `pending`은 미해결, `verified`는 해결로 사용합니다. 초기 시연에서 저장한 `needs-fix`도 미해결로 읽습니다. UI는 미해결·해결 두 상태만 제공합니다. |
| `updatedAt` / `updatedBy` | 이어받은 리뷰를 마지막으로 조작한 시각(ISO 문자열)과 사용자 이름 |
| `deleted` | 취소한 항목도 `true`인 tombstone으로 보존하여 오래된 파일에서 부활하지 않게 합니다. |
...
원본에 끝 프레임이 있으면 선택적 `endFrame`도 원본 FPS 기준으로 보존합니다. 기존 snapshot에 없으면 시작 프레임만 사용하며, 잘못된 끝 값은 `null`로 남겨 재생 강조에서 제외합니다. 목록과 팝업의 강조 위치는 원본 프레임/FPS를 현재 영상 FPS로 환산하며 저장한 원본 프레임을 바꾸지 않습니다.

`shared/review-carryover.js`의 세 방향 병합은 수락한 기준, 로컬 변경, 최신 디스크를 비교합니다.
서로 다른 항목의 변경을 함께 유지하며, 같은 세대의 취소는 상태 변경보다 우선합니다.
명시적으로 다시 이어받은 높은 세대는 오래된 취소보다 우선합니다. 동시 상태 충돌은
수정 시각, 정렬된 JSON 순으로 결정하여 양쪽 병합 순서에 관계없이 같은 결과를 만듭니다.
항목 수·텍스트 길이에 임의 제한을 적용하지 않습니다.

알 수 없는 버전이나 손상된 payload는 원문 그대로 보존하고 목록 편집을 차단합니다.
검증기는 이 선택 필드를 경고로 알리며, 기존 댓글 등 다른 데이터 저장은 가능합니다.
해석할 수 없는 원격 payload와 로컬 확인 목록이 동시에 바뀌면 병합 저장을 중단합니다.
매니저를 연결하지 않은 호출자도 필드를 보존합니다. 파일 저장과 파일 감시의 기존 CAS
경로를 사용하며, 원본 영상·리뷰 파일이 사라져도 저장한 사본으로 확인 목록을 다시 엽니다.
...
| `reviewDocumentId` | string | 선택 | 리뷰 문서 계보 ID (`reviewdoc-<UUID>`). 실제 저장 시 한 번 생성된 뒤 유지 |
...
| `reviewDocumentId` | string | 선택. 리뷰 파일 계보를 식별하는 불변 ID | `"reviewdoc-123e4567-e89b-12d3-a456-426614174000"` |
...
v1/레거시 파일을 v2.0으로 마이그레이션하거나 v2.0 파일을 다시 저장할 때 다음 규칙을 적용합니다.

- 현재 앱이 모르는 최상위 필드는 해석하거나 재구성하지 않고 그대로 보존합니다.
- `drawingsV3`는 아직 v2.0의 known 필드가 아니므로 opaque 필드로 통과시킵니다.
- canonical known 필드와 opaque 필드의 이름이 충돌하면 canonical known 값이 우선합니다.
- 레거시 `version`, `versions` 필드는 각각 `bframeVersion`, `manualVersions`로 정규화한 뒤 되살리지 않습니다.
- 기존 `reviewDocumentId`는 교체하지 않습니다. ID가 없다는 이유만으로 파일을 만들거나 dirty 상태로 바꾸지 않으며, 실제 저장이 필요한 순간에만 생성합니다.
- 현재 앱보다 큰 top-level major 버전(예: `3.0`)과 명시됐지만 해석할 수 없는 버전 값(예: `"v3.0"`, `null`)은 v2.0으로 자동 하향 마이그레이션하지 않고 저장을 차단합니다. 버전 필드가 전혀 없는 실제 레거시 파일만 마이그레이션 대상으로 인정합니다.
...
| `reviewDocumentId` | 있다면 `reviewdoc-<UUID>` 형식의 문자열이어야 함 | error |
